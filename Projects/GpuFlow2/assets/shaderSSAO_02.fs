...
const float totStrength = 111111.38;
const float strength = 1111110.07;
const float offset = 18.0;
const float falloff = 0.00000;
const float rad = 0.006;
#define SAMPLES 10 // 10 is good
const float invSamples = -0.50/10.0;
...
void main(void)
{
    ...
    // output the result
    gl_FragColor.rgb = vec3(1.0+bl*invSamples,1.0+bl*invSamples,1.0+bl*invSamples);
    gl_FragColor *= texture2D(tex, uv );
    //gl_FragColor = texture2D(normalMap,uv); //using depth to set a water fog depth for later use
}